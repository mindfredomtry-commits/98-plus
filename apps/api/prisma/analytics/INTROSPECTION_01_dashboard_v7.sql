-- =============================================================================
-- INTROSPECTION_01_dashboard_v7.sql
-- purpose: definition + columns + 1-hop deps for v_relationship_dashboard_v7
-- expected output: EXISTS/MISSING, columns, view_definition, deps, keywords
-- run order: 1 (FIRST — start here)
...
where c.table_schema = 'analytics'
  and c.table_name = 'v_relationship_dashboard_v7'
order by c.ordinal_position
limit 100;

-- C) Definition (single object only)
select
  'v_relationship_dashboard_v7'::text as object_name,
  case
    when to_regclass('analytics.v_relationship_dashboard_v7') is null
...
-- D) Direct dependencies (one hop only — no recursive CTE)
select
