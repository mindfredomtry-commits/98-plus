...
function logEmptyShellRootTrace(data: Record<string, unknown>): void {
  if (typeof window === 'undefined') return;
  window.__debug98log?.('[EMPTY SHELL ROOT TRACE]', data);
}

function logShellRenderTrace(data: Record<string, unknown>): void {
  if (typeof window === 'undefined') return;
  window.__debug98log?.('[SHELL RENDER TRACE]', data);
}

function overlayQueueHeadBanIdFromHead(
...
function ProvidersBody({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (!shouldMountNotificationOverlayHost) return;

    const effectiveKind = effectiveNotificationQueueShellKind;
    const hasCheckBan = Boolean(checkBan?.id ?? checkBanRef.current?.id);
    const hasActiveResultPayload = Boolean(activeResultPayload?.id);
    const shouldRenderGlobalOverlayHost =
      !composeBlocksNotificationHost && shouldMountNotificationOverlayHost;
    const shouldRenderNotificationQueueShell = shouldRenderGlobalOverlayHost;

    let childrenBranchName = 'none';
    let hasJsxChild = false;
    if (queueShellShowsResult && activeResultPayload) {
      childrenBranchName = 'result-overlay';
      hasJsxChild = true;
    } else if (effectiveKind === 'check' && !showCheckOverlayDirect) {
      childrenBranchName = 'check-overlay';
      hasJsxChild = true;
    } else if (
    ...
    const shellTracePayload = {
      hasJsxChild,
      effectiveKind,
      effectiveNotificationQueueShellKind: effectiveKind,
      notificationQueueShellKind,
      queueShellShowsResult,
      hasCheckBan,
      checkBanId: checkBan?.id ?? null,
      checkBanRefId: checkBanRef.current?.id ?? null,
      hasActiveResultPayload,
      activeResultPayloadBanId: activeResultPayload?.id ?? null,
      resultRefBanId: resultRef.current?.id ?? null,
      displayResultBanId: displayResult?.id ?? null,
      queueHeadKind: queueHead?.kind ?? null,
      queueHeadBanId: overlayQueueHeadBanIdFromHead(queueHead),
      heldKind: held?.kind ?? heldUserCardOverlay?.kind ?? null,
      heldBanId: held
        ? heldUserCardBanId(held)
        : heldUserCardOverlay
          ? heldUserCardBanId(heldUserCardOverlay)
      ...
      shouldRenderGlobalOverlayHost,
      shouldRenderNotificationQueueShell,
      childrenBranchName,
    };

    logShellRenderTrace(shellTracePayload);

    if (!hasJsxChild) {
      logEmptyShellRootTrace({
        ...shellTracePayload,
        emptyShellRootCause:
          chainAdvanceWaiting && chainAdvancePlaceholderKind
            ? `chainAdvanceWaiting-placeholder-${chainAdvancePlaceholderKind}`
            : chainAdvanceWaiting
              ? 'chainAdvanceWaiting'
              : notificationChainTransitioning
                ? 'notificationChainTransitioning'
                : effectiveKind === 'result'
                  ? 'result-shell-without-jsx-child'
                  : effectiveKind === 'check'
                    ? 'check-shell-without-jsx-child'
                    : effectiveKind === 'incoming'
                      ? 'incoming-shell-without-jsx-child'
                      : 'host-without-jsx-child',
      });
    }
  }, [
    activeResultPayload,
