...
export type OwnerFalseWhileActiveCheckDecisionInput = {
  ...
  reason: string;
  calledFrom: string;
  snapshot: Partial<OwnerFalseWhileActiveCheckSnapshot> & {
    ownerHasPendingNotificationChain: boolean;
    legacyHasPendingNotificationChain: boolean;
  };
...
export type OwnerFalseWhileActiveCheckBranchTakenInput = {
  decisionName: string;
  branchTaken: string;
  source: string;
  reason: string;
  calledFrom: string;
  snapshotBefore: OwnerFalseWhileActiveCheckTraceSnapshot;
...
function hasCompleteOwnerLegacyPatch(
  patch?: Partial<OwnerFalseWhileActiveCheckSnapshot>,
): patch is Partial<OwnerFalseWhileActiveCheckSnapshot> & {
  ownerHasPendingNotificationChain: boolean;
  legacyHasPendingNotificationChain: boolean;
} {
  return (
    patch != null &&
    typeof patch.ownerHasPendingNotificationChain === 'boolean' &&
    typeof patch.legacyHasPendingNotificationChain === 'boolean'
  );
}
...
function mergeSnapshot(
  ...
  const base = hasCompleteOwnerLegacyPatch(patch)
    ? emptySnapshot()
    : (hooks?.readSnapshot() ?? emptySnapshot());
...
export function observeOwnerFalseWhileActiveCheckDecision(
  ...
  const snap = mergeSnapshot(input.snapshot);
...
export function getStoredOwnerFalseWhileActiveCheckDecisionTrace(
  decisionName: string,
  checkBanId: string | null,
): OwnerFalseWhileActiveCheckTraceSnapshot | null {
  return storedDecisionTraceByKey.get(oneShotDecisionKey(decisionName, checkBanId)) ?? null;
}

export function noteOwnerFalseWhileActiveCheckBranchTaken(
  input: OwnerFalseWhileActiveCheckBranchTakenInput,
): void {
  if (!isClientDiagTraceEnvironment()) return;

  const decisionKey = oneShotDecisionKey(
    input.decisionName,
    input.snapshotBefore.checkBanId,
  );
  ...
  const snapshotBefore = input.snapshotBefore;
  const afterSnap = input.snapshotAfter
    ? mergeSnapshot(input.snapshotAfter)
    : mergeSnapshot({
        ownerHasPendingNotificationChain:
          snapshotBefore.ownerHasPendingNotificationChain,
        legacyHasPendingNotificationChain:
          snapshotBefore.legacyHasPendingNotificationChain,
      });
